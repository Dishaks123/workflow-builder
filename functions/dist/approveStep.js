...
async function approveStep(req, res) {
    ...
    try {
        const runId = req.body?.input?.run_id;
        ...
        if (!runId && !stepRunId) {
            return res.status(400).json({
                success: false,
                message: "run_id or step_run_id is required"
            });
        }
        /*
        ...
        if (approval.step_status !==
            "paused") {
            return res.status(409).json({
                success: false,
                message: "Approval step is not waiting for approval"
            });
        }
        /*
         * Layer 1:
        ...
        if (role !== "owner" &&
            ...
            return res.status(403).json({
                success: false,
                message: "Only owner or editor can approve this step"
            });
        }
        /*
         * Record the approval.
      ...
      `, [
        ...
        return res.status(200).json({
            ...
            approved_by: userId,
            role,
            message: "Approval recorded and workflow resumed"
        });
    }
    catch (error) {
